...
operator=(const rb_tree<Key, Value, KeyOfValue, Compare>& x) {
    if (this != &x) {
        // can not be done as in list because Key may be a constant type
        erase(begin(), end());
        root() = __copy(x.root(), header);
        if (root() == NIL) {
            leftmost() = header;
...
rb_tree<Key, Value, KeyOfValue, Compare>::
__insert(link_type x, link_type p, const Value& v) {
    ++node_count;
    link_type z = get_node();
    construct(value_allocator.address(value(z)), v);
    if (p == header || x != NIL || key_compare(KeyOfValue()(v), key(p))) {
        left(p) = z;  // also makes leftmost() = z when p == header
        if (p == header) {
            root() = z;
            rightmost() = z;
        } else if (p == leftmost())
...
void rb_tree<Key, Value, KeyOfValue, Compare>::erase(iterator position) {
    ...
    if (one != z) { // relink one in place of z
        parent(left(z)) = one; 
        left(one) = left(z);
        if (one != right(z)) {
            parent(x) = parent(one); // possiblone x == NIL
            left(parent(one)) = x;   // one must be a left child
            right(one) = right(z);
            parent(right(z)) = one;
        } else
            parent(x) = one;  // needed in case x == NIL
        if (root() == z)
            root() = one;
            ...
            right(parent(z)) = one;
        parent(one) = parent(z);
        ::swap(color(one), color(z));
        one = z;
                       // one points to node to be actually deleted
    } else {  // one == z
        parent(x) = parent(one);   // possibly x == NIL
        if (root() == z)
            root() = x;
        else 
            if (left(parent(z)) == z)
